...
<script>
     $(document).ready(function (){
        ...
        $(document).on('click', '.slot-duration-delete-btn', function (e) {
          e.preventDefault();
          ...
          swal({
              title: "Are you sure?",
              text: "Once deleted, you will not be able to recover this slot_duration",
              icon: "warning",
              buttons: true,
              dangerMode: true,
            })
            .then((willDelete) => {
              if (willDelete) {
                  $.ajax({
                    type: "POST",
                    url: "slot-duration-code.php",
                    data:{
                      "delete_btn_set":1,
                      "delete_id": delete_id,
                    },
                    success: function (responses){
                      swal("slot_duration Deleted Successfully!", {
                        icon: "success",
                      })
                      .then((result) => {
