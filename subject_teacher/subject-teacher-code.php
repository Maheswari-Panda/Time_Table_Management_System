...
if (isset($_POST['subject-teacher-add-btn'])) {
    ...
    if ($result) {
        ...
        header('Location: ../subject_teacher/subject-teacher-details.php'); // Redirect to a details page for subject-teacher
    } else {
        $_SESSION['status'] = "Error assigning teacher: " . mysqli_error($conn);
        $_SESSION['status_code'] = "error";
        header('Location: ../subject_teacher/subject-teacher-add.php'); // Redirect back to the add page
    }
    exit();
}
...
if (isset($_POST['subject-teacher-update-btn'])) {
    ...
    if ($result) {
        ...
        $_SESSION['status_code'] = "success";
        header('Location: ../subject_teacher/subject-teacher-details.php'); // Redirect to details page
    } else {
        $_SESSION['status'] = "Error updating relationship: " . mysqli_error($conn);
        $_SESSION['status_code'] = "error";
        header('Location: ../subject_teacher/subject-teacher-edit.php'); // Redirect back to edit page
    }
    exit();
}
...
if (isset($_POST['delete_btn_set'])) {
    ...
    if ($result) {
        ...
        $_SESSION['status_code'] = "error";
        header('Location: ../subject_teacher/subject-teacher-details.php'); // Redirect back to details page
    }
    exit();
}
